...
| AWS Config recorder, per account (`terraform/aws/modules/config_recorder`) | A real, queryable resource-configuration history and drift record — not just an org-wide aggregator with nothing feeding it. Extracted from a real production fix: the aggregator existed for a long time before any account actually had a recorder running. | NIST CSF **Identify (ID.AM)**, **Detect (DE.CM)**; SOC 2 **CC7.1** |
